...
c = c.replace(/await screen\.findByRole\("heading",\s*\{\s*name: "Slime on the Verge"(?:,\s*level: 2)?\s*\}\s*\)/g, 'await screen.findByText(/Slime on the Verge/i)');
c = c.replace(/screen\.getByRole\("heading",\s*\{\s*name: "Slime on the Verge"(?:,\s*level: 2)?\s*\}\s*\)/g, 'screen.getByText(/Slime on the Verge/i)');

// 6. Gained XP text length issue
c = c.replace(/expect\(screen\.getAllByText\(\/Gained 45 XP and 18 gold\/\)\)\.toHaveLength\(2\);/g, 'expect(screen.getByText(/Gained 45 XP and 18 gold/)).toBeInTheDocument();\n    expect(screen.getByText(/Victory! \\+45 XP/)).toBeInTheDocument();');
c = c.replace(/expect\(screen\.getAllByText\(\/The party was defeated\.\/\)\)\.toHaveLength\(2\);/g, 'expect(screen.getByText(/The party was defeated\\./)).toBeInTheDocument();\n    expect(screen.getByText(/Defeated!/)).toBeInTheDocument();');
c = c.replace(/Gained 10 XP and 4 gold\./g, 'Victory! +10 XP, +4 Gold.');
c = c.replace(/The party was defeated\. The character returns with 1 HP\./g, 'Defeated!');
...
c = c.replace(/await screen\.findByRole\("heading", \{ name: "Aster Vale" \}\)/g, 'await screen.findByText("Aster Vale")');
c = c.replace(/screen\.getByRole\("heading", \{ name: "Aster Vale" \}\)/g, 'screen.getByText("Aster Vale")');
